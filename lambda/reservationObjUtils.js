...
async function getFutureReservationObjects(parkName, facilityName) {
  ...
  try {
    logger.debug('reservationsObjectQuery', reservationsObjectQuery);
    futureResObjects = await runQuery(reservationsObjectQuery);
    logger.debug('futureResObjects:', futureResObjects);
  } catch (error) {
    logger.error('Error in getFutureReservationObjects():', error);
    ...
    for (let j = 0; j < timesToUpdate.length; j++) {
      ...
      if (newResAvailability < 0) {
        try {
          ...
          logger.error('Error occured while executing updatePassObjectsAsOverbooked():', error);
          throw error;
        }
      } else {
        ...
        try {
          ...
          logger.error('Error occured while executing checkForOverbookedPasses():', error);
          throw error;
        }
        if (overbookedPasses.length > 0) {
          try {
            ...
            logger.error('Error occured while executing reverseOverbookedPasses():', error);
            throw error;
          }
        }
      }
...
async function updatePassObjectsAsOverbooked(facilityName, shortPassDate, type, numberOfPassesOverbooked) {
  ...
  try {
    ...
    logger.debug('passes:', passes);
  } catch (error) {
    logger.error('Error occured while getting overbooked passes in updatePassObjectsAsOverbooked():', error);
    throw { msg: 'Something went wrong.', title: 'Operation Failed' };
  }
  ...
  for (let i = 0; i < overbookedPasses.length; i++) {
    ...
    try {
      logger.debug('updatePassObject:', updatePassObject);
      const res = await dynamodb.updateItem(updatePassObject).promise();
      logger.debug('Pass set to overbooked', res);
    } catch (error) {
      logger.error('Error occured while updating pass in updatePassObjectsAsOverbooked():', error);
      throw { msg: 'Something went wrong.', title: 'Operation Failed' };
    }
  }
  // Return remainder.
...
async function checkForOverbookedPasses(facilityName, shortPassDate, type) {
  ...
  try {
    ...
    logger.error('Error occured while getting overbooked passes in reverseOverbookedPasses():', error);
    throw { msg: 'Something went wrong.', title: 'Operation Failed' };
  }
  return passes;
}
...
async function reverseOverbookedPasses(passes, passDiff) {
  ...
  for (let i = 0; i < passes.length; i++) {
    ...
    try {
      ...
      logger.error('Error occured while updating pass in reverseOverbookedPasses():', error);
      throw { msg: 'Something went wrong.', title: 'Operation Failed' };
    }
  }
  return passTally;
